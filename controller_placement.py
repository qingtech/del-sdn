...
#l_wei[i][j]表示该链路的“距离”，交换机与控制器的通讯总是走“距离”最短的路径，使用Floyd算法得到
#输出：
#i：与控制器直接相连的交换机
def controller_place(s_wei,l_wei):
	#数据合法性检验
	sn = len(s_wei)
	if sn < 0:
...
if __name__ == '__main__':
	...
	'
	s_wei = [2,2,10,2]
	l_wei = [[0,1,1,1],[max,0,1,max],[1,max,0,max],[max,max,1,0]]
	i = controller_place(s_wei, l_wei)
	print 'i=%d'%i
